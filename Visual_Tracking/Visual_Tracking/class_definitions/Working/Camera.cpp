	...
	: CameraStateSpace(tVal.val[0], tVal.val[1], tVal.val[2], eulerAnglesVal.val[0], eulerAnglesVal.val[1], eulerAnglesVal.val[2], stateVal)
{
	...
	focal_PIxels = checkFocal(focal_PIxelsVal);
...
{
	fov = fmod(fmod(fovVal, _DEG) + _DEG, _DEG);
	focal_PIxels = static_cast<float>(width) / (2.f * tan(deg2rad(fov / 2.f)));
	focalMetric = focal_PIxels / _Kp;
}
...
// Change focal_PIxels
void Camera::setFocal_PIxels(float focal_PIxelsVal)
{
	focal_PIxels = checkFocal(focal_PIxelsVal);
	focalMetric = focal_PIxels / _Kp;
	fov = 2.f * rad2deg(atan2f(static_cast<float>(width), 2.f * focal_PIxels));
}

// Return focal_PIxels
float Camera::getFocal_PIxels() const
{
	return focal_PIxels;
}
...
{
	return (Mat_<float>(3, 3) << focal_PIxels, 0.f, principalPoint.x, 0.f, focal_PIxels, principalPoint.y, 0.f, 0.f, 1.f);
}
